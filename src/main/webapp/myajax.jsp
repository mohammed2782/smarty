...
<%@ page
	import="smarty.security.*, java.util.List , java.util.HashMap , java.util.ArrayList , java.util.Iterator , java.util.LinkedHashMap , java.util.Enumeration
			,java.sql.Connection , java.sql.PreparedStatement , java.sql.ResultSet , smarty.core.html.*"%>
...
try{ 
	...
	if (!queryParametersEmpty){
		conn = mysql.getConn();
		System.out.println("sqllookup-->"+sqllookup);
		pst = conn.prepareStatement(sqllookup);
		rs = pst.executeQuery();
	}else{
		rs = null;
	}
	if (HTMLtype.equals("DROPLIST") || HTMLtype.equals("CLASSIC_SELECT") ){
		...
		HTMLResult =(chkbx.getHtmlInput(new HashMap<String , Integer>() ,new HashMap<String , String>() , 
					colMapValues , new ArrayList<String>(),"",false , "" , false , false , false, null, null, null)).toString();
	}else{
