...
<%! public void Render(CoreMgr mgr , 
	...
	try{
		...
		if (whichAction!=null){
			...
			}else if (request.getParameter("smarty_newformbtn")!=null){
				if (request.getParameter("smarty_newformbtn").equalsIgnoreCase("newform")){
					mgr.setDisplayMode("NEWSINGLE"); 
				}
		...
		if (request.getParameter("op")!= null){
 		...
 		if (request.getParameter("smartyValidationMsg")!=null && request.getParameter("myClassBean").equals(className)){
 			myout.println("<div align='center'><p><font color='red'>"+URLDecoder.decode(request.getParameter("smartyValidationMsg"),java.nio.charset.StandardCharsets.UTF_8.toString())+"</font></p></div>");
 		}
		if (mgr.isInsertAction()){
			whichAction = null;
			ValidationMsg= mgr.doInsert(request,true);
			if (ValidationMsg.equalsIgnoreCase("Error"))
				myout.println("<div align='center'><h3><span class='badge bg-danger'>حصل خطأ لم يتم الحفظ</span></h3></div>");
			else
				myout.println("<div align='center'><h3><span class='badge bg-success'>"+ValidationMsg+"</span></h3></div>");
			whichAction = "new";
			...
			if (!mgr.isInsertErrorFlag()){
				...
				String classNameUrl = URLEncoder.encode(className, java.nio.charset.StandardCharsets.UTF_8.toString());
				myout.println("<script>location.replace('?myClassBean="+classNameUrl+"&smartyValidationMsg="+encodedRedirectUrl+"');</script>");
				//response.sendRedirect("?myClassBean="+classNameUrl+"&smartyValidationMsg="+encodedRedirectUrl);
				return;
			} 
		}
...
public void showUpdForm( CoreMgr mgr ,  JspWriter myout , HttpServletRequest request){
	...
	try{
		myout.print(mgr.getUpdForm(request,null));  
	}catch(Exception e){
		e.printStackTrace();
		System.out.println("Error at showNewForm Method-Main.jsp");
	}
