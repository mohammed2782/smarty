...
<%@ page import="com.app.cases.NewCasesByState" %> 
...
$(document).ready(function() {
	$('#add_rcv_dtls').click(function(){
		...
		$.get('../../MultiRowsByStateSRVLT?state='+state+'&loadRcvRow='+RCVno , dataToSend,function(data, status){ 
			if (status=='success'){
...
function loadDistrict(seq){
	...
	+ "(select cdi_id, cdi_name from kbcity_district where  cdi_stcode = '"+destCity+"' and cdi_name='بلا عنوان' " 
...
function loadAgent(seq){
	...
	var lookupSql = "select us_id , us_name from kbusers where us_rank='DLVAGENT' "
		+ " and us_to_state like '%"+destCity+"%'  order by us_name ";
...
function disableF5(e) {
	  if ((e.which || e.keyCode) == 116){
		  e.preventDefault();
		  document.getElementById('save_new_form_com_dot_app_dot_cases_dot_NewCasesByState').click();
	  }
	};
