...
com.app.bussframework.SingleQueue_INIT_NEWINSTORE_DIRECT_BRANCH_ASSIGN,
java.sql.Connection" %>
...
<%
	...
	SingleQueue_INIT_NEWINSTORE_DIRECT_BRANCH_ASSIGN DirectBranchAssign = new SingleQueue_INIT_NEWINSTORE_DIRECT_BRANCH_ASSIGN();
	Render(DirectBranchAssign , out , request, response , Myglobals , objectState , pageName2);
%> 
<script>
...
function getrcpNo(){
	...
	$.get('../../getAllReceiptInfoOfSameReceiptNumberInQueueSRVL?ReceiveAnyShipment=assignToBranch&stage=INIT&step=NEWINSTORE&c_custreceiptnoori='+barcodeScanned_NEW_INSTORE ,function(data, status){
		if (status=='success'){
			check = false;
...
function addRow(data, barcodeScanned_NEW_INSTORE){		
	...
	hiddenFields += "<input type='hidden'  id='q_assigned_to_smartyrow_"+rowNo+"' name='q_assigned_to_smartyrow_"+rowNo+"' value=''/>";
	hiddenFields += "<input type='hidden' name='q_action_smartyrow_"+rowNo+"' value='ASSIGN_LIASIONAGT'/>";
	...
	$("#smarty_table_com_dot_app_dot_bussframework_dot_SingleQueue_INIT_NEWINSTORE_DIRECT_BRANCH_ASSIGN tr:first").after(row);
	rowNo ++;
	input.value = '';
}
...
$(document).ready(function() {
	$("#globalagentselect").on('change', function() {
		...
		for (i = 1 ; i <= $('#smartyhiddenmultieditrowsno').val() ; i++){
			$('#q_assigned_to_smartyrow_'+i).val(globalAgentid);
		}
	});
});
