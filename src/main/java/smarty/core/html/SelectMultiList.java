...
public class SelectMultiList extends HTMLShapes{
	...
	public StringBuilder getHtmlInput(HashMap<String, Integer> sqlColsSizes,
			...
			,boolean multiEdit, HashMap<String,String> minValMap,
    	...
    	String classSelect2= "select2";
    	if (userDefinedColsHtmlType.get(colName).equalsIgnoreCase("CLASSIC_MULTILIST")) {
    		classSelect2 = "";
    	}
		if (required)
		...
		StringBuilder html = 
				new StringBuilder("<select id='"+colNameToSave+"' name='"+colNameToSave+"'"
						+ " "+req+" class=' "+classSelect2+" form-control' multiple='multiple' "+Disabled+" style='"+style+"'>");
