...
public class TextArea extends HTMLShapes{
	...
	public StringBuilder getHtmlInput(
    	...
    	if (! hidden){
    		TextHtml.append("<textarea "+htmlAttr+" name ='"+colNameToSave+"' id='"+colNameToSave+"'  rows='2' cols='20' "+Readonly+" "+Disabled+
			           "class='"+shapeClass+"' style='"+style+"' "+req+" "+onFocus+" "+onBlur+" "+maxLengthString+">"+DisplayValue+"</textarea>");
    	}else{
    		TextHtml.append("<input "+htmlAttr+" type='hidden'   name ='"+colNameToSave+"' id='"+colNameToSave+"' value='"+DisplayValue+"'></input>");
    	}
