...
public class Password extends HTMLShapes {
	...
	public StringBuilder getHtmlInput( 	HashMap<String , Integer> sqlColsSizes , 
		...
		style = "text-align:"+textAlign+"; background-color:black;  ";
		
		if (! hidden){
			TextPassword.append("<input "+htmlAttr+" type='password' style='"+style+"'  name ='"+colNameToSave+"' " +
		     		"id='"+colNameToSave+"' value='"+DisplayValue+"' class='"+shapeClass+"' "+Readonly+" "+Disabled
		     		+" "+onFocus+" "+onBlur+" "+maxLengthHtml+" "+inputSizeHtml+" "+req+">");
		}else{
			TextPassword.append("<input "+htmlAttr+" type='hidden'   name ='"+colNameToSave+"' id='"+colNameToSave+"' value='"+DisplayValue+"'></input>");
		}
