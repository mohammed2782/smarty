...
public class Menu {
	...
	public StringBuilder loadMenu(String mainProjPath){
		...
		sb.append(" <ul id=\"respMenu\" class=\"horizontal-menu\">"); 
		try{
			 conn = mysql.getConn();
			 ...
			 while (rsMenu.next()){
				...
				sb.append("<li><a href=\"javascript:;\">"
						+ "<i class=\""+rsMenu.getString("mt_iconclass")+"\" aria-hidden=\"true\"></i><span class=\"title\">"+rsMenu.getString("mt_name")+"</span> "
						 ...
						 while (rsSubMenu.next()){
							 if (listsubMenuIdPerms.contains(rsSubMenu.getString("sm_id"))) {
								 sb.append("<li><a href='../"+rsMenu.getString("mt_code")+"/"+rsSubMenu.getString("sm_submenucode")+".jsp'>"
										 +"<i class=\"zmdi zmdi-dot-circle-alt\"></i>"+rsSubMenu.getString("sm_submenu_name")+"</a></li>");
							 }
						 }
