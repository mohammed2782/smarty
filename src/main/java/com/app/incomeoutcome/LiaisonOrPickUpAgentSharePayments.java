...
public class LiaisonOrPickUpAgentSharePayments extends CoreMgr{
	private FinOperationEntity entityType;
	...
	public LiaisonOrPickUpAgentSharePayments() {
		...
		userDefinedGridCols.add("lpas_tot_cases_center");
		userDefinedGridCols.add("lpas_tot_cases_rural");
		userDefinedGridCols.add("lpas_share_center");
		userDefinedGridCols.add("lpas_share_rural");
		userDefinedGridCols.add("lpas_share_calculated_profit_center");
		userDefinedGridCols.add("other_share_center");
		userDefinedGridCols.add("lpas_share_calculated_profit_rural");
		userDefinedGridCols.add("other_share_rural");
		...
		userDefinedColLabel.put("lpas_for_which_trans_id", "عن دفعة فرع");
		userDefinedColLabel.put("lpas_type_of_share", "نوع مبلغ الشراكة");
		userDefinedColLabel.put("lpas_share_center", "نسبة / مبلغ للمركز");
		userDefinedColLabel.put("lpas_share_rural", "نسبة / مبلغ للأطراف");
		
		userDefinedColLabel.put("lpas_tot_cases", "عدد الشحنات الكلي");
		userDefinedColLabel.put("lpas_tot_cases_center", "عدد شحنات المركز");
		userDefinedColLabel.put("lpas_tot_cases_rural", "عدد شحنات الأطراف");
		userDefinedColLabel.put("lpas_share_calculated_profit_center", "حصة الشريك لوصولات المركز");
		userDefinedColLabel.put("other_share_center", "باقي المبلغ للمركز");
		userDefinedColLabel.put("lpas_share_calculated_profit_rural", "حصة الشريك لوصولات الأطراف");
		userDefinedColLabel.put("other_share_rural", "باقي المركز للأطراف");
		...
		userDefinedNewCols.add("lpas_share_rural");
		...
		userDefinedColsMustFill.add("lpas_share_rural");
	...
	public void initialize(HashMap smartyStateMap) {
		MainSql = "select trans_id, lpas_for_which_trans_id, trans_amount_paid_actually_iqd,"
				...
				+ " where trans_operationentity='"+entityType.toString()+"'"
						+ " and trans_operationcode = 'SHARE' "
				+ " and trans_initiated_in_branch_id = {userstorecode} and lpas_for_which_branch={BRANCH_TO_PAY_TO_G} "
				+ " and trans_deleted='N' order by trans_id desc ";
		Connection conn = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		int branchPaidTo = Integer.parseInt(replaceVarsinString(" {BRANCH_TO_PAY_TO_G} ", arrayGlobals).trim());
		...
		if (FinOperationEntity.LIAISON_AGENT == entityType) {
			...
			+ " and trans_entity_id = "+branchPaidTo+" and trans_operationcode = 'CASES' "
			+ " and trans_initiated_in_branch_id = "+branchId_G+""
			+ " and trans_id not in (select lpas_for_which_trans_id from p_liaison_pickup_agent_share)"
			+ " and trans_deleted = 'N' ");
		}
		super.initialize(smartyStateMap);
		userDefinedCaption = "دفعات مندوب الأرتباط لفرع  "+receiverBranchName;
	}
	...
	public String doDelete(HttpServletRequest rqs) {
		...
		try {
			...
			UtilitiesStandardFinancials.deleteTransaction(conn, transactionToBeDeleted, userId_G, branchId_G, 
					"مسح عملية مالية - حسابات مندوب الأرتباط ");
	...
	private HashMap<String, Long> calculateTotalShareAmtFromBranchPayment (Connection a_conn, int a_transId, boolean a_isCenter) throws Exception{
		PreparedStatement ps = null;
		ResultSet rs = null;
		 HashMap<String, Long> map = new  HashMap<String, Long>();
		try {
			ps = a_conn.prepareStatement("select count(*),  sum((cc_pathcost - c_agentshare)) as profit "
					+ " from p_cases join p_caseschain on c_id = cc_caseid  "
					+ " where cc_branchpmtid = ? and c_rural = ?");
	...
	public String doInsert(HttpServletRequest rqs, boolean autoCommit) {
		...
		try {
			...
			int shareAmountRural  = Integer.parseInt( inputMap_ori.get("lpas_share_rural")[0]);
			
			long centerCasesCount = 0 , ruralCasesCount = 0;
			long totalAmountCenterCases = 0, totalAmountForRuralCases=0,
					totalShareAmountToPayForCenterCases =0 , totalShareAmountToPayForRuralCases = 0;
			if(entityType == FinOperationEntity.LIAISON_AGENT) {
				HashMap<String, Long> centerCasesMap = 
						calculateTotalShareAmtFromBranchPayment (conn, paymentForTransactionId, true);
				totalAmountCenterCases = centerCasesMap.get("total_amount");
				centerCasesCount =  centerCasesMap.get("total_cases");
				
				HashMap<String, Long> ruralCasesMap  = 
						calculateTotalShareAmtFromBranchPayment (conn, paymentForTransactionId, false);
				totalAmountForRuralCases = ruralCasesMap.get("total_amount");
				ruralCasesCount = ruralCasesMap.get("total_cases");
			}
			if (shareTypePercentageOrDirectAmount.equalsIgnoreCase("PERCENTAGE")) {
				...
				totalShareAmountToPayForRuralCases = ((totalAmountForRuralCases * shareAmountRural) / 100);
			}
			SafePaymentTypeMetaInfoBean safePaymentTypeMetaInfoBean = 
							UtilitiesSafeFinancials.getSafePaymentTypeMetaInfoKbgeneral(conn, 
									entityType.toString(), "PMTTYPE" , FinOperationCode.SHARE.toString());
			...
			int accountBoxTransactionId = UtilitiesSafeFinancials.createAcctBoxTransactions(
					...
					userId_G, (totalShareAmountToPayForCenterCases + totalShareAmountToPayForRuralCases), 
					0, branchId_G, safePaymentTypeMetaInfoBean.getName() + " - " + " - دفع مبلغ مالي", 
					"kbbranches", "branch_id",  branchPaidTo, "branch_name",
					Utilities.getBranchesInfo(conn, branchPaidTo+"").get("name"), userId_G);
					
			StandardTransactionBean standardTransactionBean = new StandardTransactionBean();
			standardTransactionBean.setEntity(entityType);
			standardTransactionBean.setEntityId(branchPaidTo);
			standardTransactionBean.setCategory(FinOperationCategory.PMTTYPE);
			standardTransactionBean.setCode(FinOperationCode.SHARE);
			standardTransactionBean.setInitiatedInBranchId(branchId_G);
			standardTransactionBean.setWhichScreen("دفع حصة مندوب أرتباط");
			standardTransactionBean.setTransactionAmountIqd((totalShareAmountToPayForCenterCases + totalShareAmountToPayForRuralCases));
			standardTransactionBean.setAmountPaidActuallyIqd((totalShareAmountToPayForCenterCases + totalShareAmountToPayForRuralCases));
			...
			+ " lpas_tot_cases_rural		, lpas_type_of_share				 , lpas_share_center	   , "
			+ " lpas_share_rural    		, lpas_share_calculated_profit_center, lpas_share_calculated_profit_rural, "
			+ " lpas_for_which_trans_id		, lpas_for_which_branch			 	 , lpas_total_calculated_profit_center,"
			+ " lpas_total_calculated_profit_rural) "
			+ " values("+CoreUtilities.getQuestionMarks(13)+") ");
			ps.setInt(1, standardStransactionId);
			ps.setLong(2, centerCasesCount+ruralCasesCount);
			ps.setLong(3, centerCasesCount);
			ps.setLong(4, ruralCasesCount);
			ps.setString(5, shareTypePercentageOrDirectAmount);
			ps.setLong(6, shareAmountCenter);
			ps.setLong(7, shareAmountRural);
			ps.setLong(8, totalShareAmountToPayForCenterCases);
			ps.setLong(9, totalShareAmountToPayForRuralCases);
			ps.setInt(10, paymentForTransactionId);
			ps.setInt(11, branchPaidTo);
			ps.setLong(12, totalAmountCenterCases);
			ps.setLong(13, totalAmountForRuralCases);
			ps.executeUpdate();
			conn.commit();
		} catch (Exception e) {
