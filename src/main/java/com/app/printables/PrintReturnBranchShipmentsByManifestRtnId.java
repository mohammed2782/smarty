...
public class PrintReturnBranchShipmentsByManifestRtnId {
...
//                    document.bottom() - 20, 0);
	...
	public void prepareDocument(  int liasionAgentId, int rtnManifestId, String ctxPath) {
		...
		try {
			conn = mysql.getConn();
			String liaisonAgentName = utf.getDriverName(conn, liasionAgentId+"");
			dlvs = utf.getRtnItemsPerBranchRtnManifestId(conn, rtnManifestId);
			
			// here generate the manifest or get it from the db for all the cases 
			Document document = new Document(PageSize.A4.rotate(), 60, 60, 20, 50);
		    PdfWriter pdf = null;
...
//	        ct.go();
            ...
            par = new Paragraph("قائمه برواجع من فرع "+dlvs.get(0).getCurrentBranchName()+" الى فرع "+dlvs.get(0).getOriginatinBranchName(),font);
	        cell = new PdfPCell(par);
 			cell.setPaddingTop(10);
 			cell.setPaddingBottom(5);
            ...
            par = new Paragraph("رقم المنفيست "+rtnManifestId,font);
	        cell = new PdfPCell(par);
 			cell.setPaddingTop(10);
 			cell.setPaddingBottom(5);
 			cell.setVerticalAlignment(Element.ALIGN_CENTER);
	        ...
	        table = getManifestTable(conn, liasionAgentId ,rtnManifestId ,font);
	        document.add(table);
	...
	private  PdfPTable getManifestTable(Connection conn,  int liasionAgentId, int rtnManifestId, Font font){
        ...
        try{
	        ...
	        relativeWidths [0] = 11; // fragile, color red
	        //relativeWidths [1] = 6; // fragile, color red
	        relativeWidths [1] = 4; // qty
	        relativeWidths [2] = 16; // rmk
	        ...
	        relativeWidths [8] = 9; // receipt no ori
...
//				table.addCell(cell);
				...
				/* not used by TL
				ph = new Phrase(numFormat.format(ci.getSentMonyToReceiver()),font);
				cell = new PdfPCell(ph);
				if (ci.getSentMonyToReceiver()>0)
					cell.setBackgroundColor(BaseColor.GREEN);
				cell.setPaddingBottom(10);
				table.addCell(cell);
				
				if (ci.getBrinBackitems().equalsIgnoreCase("Y")) {
