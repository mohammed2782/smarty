...
public class ReturnBetweenBranchesPDF {
	...
	public File getManifestTable(int ccRtnManifestId, int a_branchCode, String ctxPath) throws Exception{		
		...
		try {
			...
			List<CaseInformation> shipments =  
					Utilities.getRtnBetweenBranchesForCcRtnManifest(conn, 
							ccRtnManifestId,
							a_branchCode);
	...
	private  PdfPTable getShipmentsTable(Connection conn ,
			List<CaseInformation> a_shipments,Font font){
        ...
        try{
	        ...
	        headers.add("أنشأ في فرع");
	        headers.add("مبلغ الوصل د.ع");
	        headers.add("العنوان");
	        headers.add("تاريخ الشحنة");
        	...
        	for (CaseInformation ci  : a_shipments){
				...
				ph = new Phrase(ci.getOriginatinBranchName(),font);
				cell = new PdfPCell(ph);
				cell.setPaddingBottom(10);
				cell.setBackgroundColor(bc);
				table.addCell(cell);
