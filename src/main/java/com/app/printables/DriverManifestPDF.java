...
public class DriverManifestPDF {
	...
	class MyFooter extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            try {
            	...
            	cb.addImage(image,PageSize.A4.getWidth()-540,0,0,50,20,document.top()+20);
	...
	public void prepareDocument(  String driverId, String stgCode, String stpCode, int a_branchCode, 
		...
		if(stgCode.equalsIgnoreCase("INIT") && stpCode.equalsIgnoreCase("PRINTMANIFEST")) {
			fromINIT_PRINTMANIFEST = true;
		}
		try {
			conn = mysql.getConn();
			imgPath = ctxPath+Utilities.getBranchLogoForPrinting(conn, a_branchCode);
			if (imgPath.isEmpty()) {
				imgPath = ctxPath+"/smartyresources/app-assets/images/logo/logo-sm.png";
			}
			pst = conn.prepareStatement("select stp_name, stp_pdf_bgcolor from kbstep where stp_code=? and stp_stgcode=?");
			pst.setString(1, stpCode);
			pst.setString(2, stgCode);
			rs = pst.executeQuery();
			if(rs.next()) {
				PDFTitle = rs.getString("stp_name");
				if(rs.getString("stp_pdf_bgcolor")!=null)
					BGColor = rs.getString("stp_pdf_bgcolor");
			}
	      ...
	      /*  String months[] = {"كانون الثاني", "شباط", "أذار", "نيسان", "أيار", "حزيران", "تموز", "أب", "أيلول", 
	       ...
	       // String arMonth = months[gcalendar.get(Calendar.MONTH)];
            ...
            if(fromINIT_PRINTMANIFEST) {
            	manifestId = ut.generateDlvAgentManifestIdForCasesInPrintManifest(conn, Integer.parseInt(driverId), a_userId, a_branchCode);
	        ...
	        table = getManifestTable(conn, manifestId, driverId , stgCode , stpCode, a_branchCode, fromdt, todt ,font, a_userId);
	        document.add(table);
	...
	private  PdfPTable getManifestTable(Connection conn, int mainfestId, String driverId, String stgCode, 
			String stpCode,  int a_branchCode, 
			String fromdt, String todt, Font font, int userId){
        ...
        try{
        	dlvs = ut.getItemsPerDriver(conn,  driverId,  stgCode,  stpCode, a_branchCode, fromdt, todt);
        	// let it update all the cases
        	if (mainfestId>0) {
        		ut.assignManifestIdToCases(conn, mainfestId, dlvs);
        	}
	        int noOfHeaders = 0;
	        float [] relativeWidths = null;
	        boolean showSenderHp =  Utilities.shouldShowSenderHpInDlvManifest(conn, a_branchCode);
	        ...
	        if(fromINIT_PRINTMANIFEST) {
	        	if (!showSenderHp) {
			        ...
			        relativeWidths [1] = 12; // amount to collect
			        relativeWidths [2] = 12; // amount to collect
			        relativeWidths [3] = 15; // sender name
			        relativeWidths [4] = 12; // rcv phone
			        relativeWidths [5] = 7; // 	receipt no ori       
	        	...
	        	if (!showSenderHp) {
	        		relativeWidths [0] = 10; // manifest id
			        relativeWidths [1] = 18; // rmk
			        relativeWidths [2] = 12; // amount to collect
			        relativeWidths [3] = 12; // amount to collect
			        relativeWidths [4] = 15; // sender name
			        relativeWidths [5] = 12; // rcv phone
			        relativeWidths [6] = 9; // 	receipt no ori       
			        relativeWidths [7] = 17; // address
			        relativeWidths [8] = 10; // date given to agent
			        ...
			        relativeWidths [2] = 12; // amount to collect
			        relativeWidths [3] = 12; // amount to collect
			        relativeWidths [4] = 10; // sender phone
			        relativeWidths [5] = 15; // sender name
			        relativeWidths [6] = 12; // rcv phone
			        relativeWidths [7] = 9; // 	receipt no ori       
			        relativeWidths [8] = 17; // address
			        relativeWidths [9] = 10; // date given to agent
			        relativeWidths [10] = 5; //seq
	        	}
	        }
	       // System.out.println("relativeWidths--->"+relativeWidths.length+", noOfHeaders--->"+noOfHeaders+", showSenderHp=>"+showSenderHp);
			...
			if(!fromINIT_PRINTMANIFEST) {
				ph = new Phrase("تاريخ الأعطاء للمندوب",font);
				cell = new PdfPCell(ph);
				cell.setBackgroundColor(BaseColor.LIGHT_GRAY);
				cell.setPaddingBottom(5);
	            table.addCell(cell);
           ...
           //3
			...
			if(!fromINIT_PRINTMANIFEST) {
				ph = new Phrase("رقم المنفيست",font);
				cell = new PdfPCell(ph);
				cell.setBackgroundColor(BaseColor.LIGHT_GRAY);
				cell.setPaddingBottom(5);
				table.addCell(cell);
        	...
        	for (CaseInformation ci  : dlvs){	
				...
				if(!fromINIT_PRINTMANIFEST) {
					ph = new Phrase(ci.getManifestId()+"",font);
					cell = new PdfPCell(ph);
					cell.setPaddingBottom(10);
					cell.setBackgroundColor(bc);
					table.addCell(cell);
