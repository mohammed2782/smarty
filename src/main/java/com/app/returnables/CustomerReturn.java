...
public class CustomerReturn extends CoreMgr{
	...
	public CustomerReturn() {
		...
		MainSql = "select concat('المتجر : ',cust_name, '<div style=\"float: left;\"><input type=\"checkbox\" "
				+ " onclick=\"checkAllCust(this,',c_custid,') \" id=\"check-customer-',c_custid,'\"></div>') as groupingcol, "
				...
				+ " c_changedprice , c_shipment_cost , c_priceb4change, c_receiptamt as currentreceiptprice,"
				+ "  c_rural , (c_receiptamt -  c_shipment_cost) as net_iqd , c_parentid "
				+ " from p_cases "
				...
				+ " where c_mastercustid ={custAccountReturnProcess} and c_pickupagent_rtnid=0  and c_cust_rtnid=0 and c_agentrtnid>0 "
		...
		userDefinedGridCols.add("currentreceiptprice");
		...
		userDefinedColLabel.put("currentreceiptprice", "مبلغ الوصل د.ع");
		userDefinedColLabel.put("net_iqd", "صافي المبلغ د.ع");
		userDefinedColLabel.put("c_shipment_cost", "أجرة التوصيل");
		...
		userDefinedSumCols.add("currentreceiptprice");
		userDefinedSumCols.add("currentreceiptprice");
		...
		userModifyTD.put("status", "modifyStatus({c_changedprice},{c_priceb4change},{c_parentid}, {currentreceiptprice},{q_stage}, {q_step},{c_rural})");
		
		userDefinedColsMustFill.add("c_mastercustid");
		userDefinedNewColsDefualtValues.put("c_mastercustid", new String[] {"{custAccountReturnProcess}"});
		userDefinedLookups.put("c_custid", "select cust_id, cust_name from kbcustomers where cust_id={custAccountReturnProcess}");
	...
	public void initialize(HashMap smartyStateMap){
		...
		int custId = Integer.parseInt(replaceVarsinString(" {custAccountReturnProcess} ", arrayGlobals).trim());
		...
		userDefinedCaption= "<div class=\"row\" style='margin-bottom: 15px;'>"
				...
				+ " id=\"allprepair\" class=\"\" onclick=\"changeToPrepairAll('all_prepair');\" type=\"checkbox\">" + 
		...
		/*userDefinedCaption= "<div class=\"row\" style='margin-bottom: 15px;'>"
				...
				+ "				</div>"
				+ "			</div>"
				+ "<div class='col-md-10 col-sm-12 col-xs-12'>"+this.userDefinedCaption+"</div>"
		...
		try {
			...
			pst = conn2.prepareStatement("select c_custreceiptnoori "
				...
				+ "  and c_allowrtncustomer='Y' and q_step in('RTN_INSTORE','PART_SUCC') and c_pickupagent_rtnid=0  and c_cust_rtnid=0 and c_agentrtnid>0 and q_branch=c_branchcode "
			...
			pst = conn2.prepareStatement("select c_rcv_hp1 "
					+ " from p_cases "
					+ " where  c_mastercustid =? "
					+ " and c_allowrtncustomer='Y' and q_step in('RTN_INSTORE','PART_SUCC') and c_pickupagent_rtnid=0  and c_cust_rtnid=0 and c_agentrtnid>0 "
					+ "  order by c_rcv_hp1");
				pst.setInt(1,custId);
				rs = pst.executeQuery();
				while (rs.next()) {
				...
				String sql = "select c_custreceiptnoori "
						+ " from p_cases "
						+ " where c_allowrtncustomer='Y' and q_step in('RTN_INSTORE','PART_SUCC') and c_mastercustid =?  and c_pickupagent_rtnid=0  and c_cust_rtnid=0 and c_agentrtnid>0 and q_branch=c_branchcode and  c_id in (";
						boolean first = true;
	...
	public String modifyStatus (HashMap<String,String> hashy) {
		...
		if (hashy.get("q_stage").equalsIgnoreCase("CNCL") ) {
			...
			System.out.println("خطأ في النظام curomer return class,modifyStatus method stage=>"+hashy.get("q_stage")+", step=>"+hashy.get("q_step"));
		}
		...
		if (colName.equalsIgnoreCase("c_custreceiptnoori") ) {
			if (!errorFlag) {
				return "<td colspan='3' stye='font-size: 15px;' align='center'>"
						+ "<form action=\"?\" id='customer-prepare-return-form' method=\"post\" style=\"display: inline;\" ><input type=\"hidden\" name=\"op\" value=\"new\"><input type=\"hidden\""
						+ " name=\"myClassBean\" value=\"com.app.returnables.CustomerReturn\">"
	...
	public String doInsert(HttpServletRequest rqs, boolean autoCommit) {
		...
		int mastercustId = Integer.parseInt(replaceVarsinString(" {custAccountReturnProcess} ", arrayGlobals).trim());
		int userStorCode = Integer.parseInt(replaceVarsinString(" {userstorecode} ", arrayGlobals).trim());
		...
		try {
			...
			if (inputMap_ori.containsKey("selectedcaseshidden") 
				...
				cidList = Utilities.SplitStringToArrayList(inputMap_ori.get("selectedcaseshidden")[0] , ",");
				 ...
				 cidList = getCid(mastercustId, conn, fromDate, toDate, userStorCode);
			...
			if (!cidList.isEmpty()) {//make sure u have cases
				...
				ps = conn.prepareStatement("select acr_id from p_customer_return where acr_closed = 'N' and acr_deleted='N' and acr_mastercustid=? ");
				ps.setInt(1, mastercustId);
				rs = ps.executeQuery();
				if(rs.next()) {
					foundOpen = true;
					acrId = rs.getInt("acr_id");
				}
				try {rs.close();} catch (Exception e) {}
				try {ps.close();} catch (Exception e) {}
				
				if (!foundOpen) {
					ps = conn.prepareStatement( "insert into p_customer_return "
							+ " 		(acr_mastercustId, acr_createddt, acr_createdby, acr_rmk)"
				...
				ps = conn.prepareStatement("update p_cases set c_cust_rtnid=?  where c_id=?");
	...
	/*private ArrayList<String> getCid(int custid, Connection conn, String fromDate, String toDate, int userBranchId) throws Exception {
		...
		try {
			...
			String querycall = "select c_id From p_cases "
				+ " where q_status !='CLS' and c_mastercustid =?  "
