...
public class setup_customers extends CoreMgr {
	public setup_customers(){		
		 ...
		 */	
		MainSql ="select kbcustomers.* , '' actions From kbcustomers where cust_mastercustid={mcust_id}";
		
		mainTable ="kbcustomers";
		keyCol = "cust_id";
		orderByCols = "cust_id desc";
		 ...
		 */
		userDefinedColLabel.put("cust_id", "إسم المتجر");
	    ...
	    userDefinedNewCols.add("cust_branch");
		...
		userDefinedNewColsDefualtValues.put("cust_mastercustid", new String[] {"{mcust_id}"});
        userDefinedReadOnlyNewCols.add("cust_branch");
		...
		//userDefinedLookups.put("cust_name", "select cust_name, cust_name from kbcustomers where cust_mastercustid={mcust_id}");
	...
	public void initialize(HashMap smartyStateMap) {
		int masterCustId = Integer.parseInt(replaceVarsinString(" {mcust_id} ", arrayGlobals).trim()); 
		userDefinedFilterLookups.put("cust_id", "select cust_id, cust_name from kbcustomers where cust_mastercustid=" + masterCustId + " ");
	...
	public String doDelete(HttpServletRequest rqs){
		...
		try {
			...
			if (!allowDelete)
				...
				String sqlSelect = "insert into deleted_customers "
							     + "(cust_id, cust_name, cust_phone1, cust_phone2, cust_createdby, cust_createddt, cust_modified, cust_modifiedby, cust_modifieddt, "
							     + "cust_discount_bgd, cust_discount_otherstates, cust_allowlogin, cust_pwd, cust_firsttimelogin, cust_pwdb4enc, "
							     + "cust_onesiganlplayerid, cust_assigned_pickup_agent_dropped, cust_showoldreceipts, cust_branch, cust_active, "
							     + "cust_mastercustid, cust_img, cust_deleted, cust_deletedby, cust_deleteddt) "
							     + "select  "
							     + "cust_id, cust_name,cust_phone1, cust_phone2, cust_createdby, cust_createddt, cust_modified, cust_modifiedby, cust_modifieddt, "
							     + "cust_discount_bgd, cust_discount_otherstates, cust_allowlogin, cust_pwd, cust_firsttimelogin, cust_pwdb4enc,  "
							     + "cust_onesiganlplayerid, cust_assigned_pickup_agent_dropped, cust_showoldreceipts, cust_branch, cust_active,  "
							     + "cust_mastercustid, cust_img, cust_deleted, ?, now() from kbcustomers where cust_id = ? ";
