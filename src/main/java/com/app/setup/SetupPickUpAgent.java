...
public class SetupPickUpAgent extends CoreMgr{
	public SetupPickUpAgent() {
		MainSql = "select us_hp,'' as customer, '' as  popup, us_id,us_createddt, us_loginid, us_password, us_active,"
				...
				+ "  , us_name, us_branchcode, us_cartype, us_carplateno "
				+ "from kbusers where  us_rank='PICKUPAGENT' and us_branchcode={userstorecode} ";
	   ...
	   userDefinedCaption = "إعدادات مندوبين الأستلام";
	   userDefinedNewCaption = "إضافة مندوب أستلام ";
	   userDefinedEditCaption = "تعديل بيانات مندوب أستلام";
				
	  
		userDefinedGridCols.add("us_loginid");
		userDefinedGridCols.add("us_id");
		...
		userDefinedColLabel.put("us_id", "كود");
		...
		userDefinedEditColsDefualtValues.put("us_rank", new String[] {"PICKUPAGENT"});
		...
		userDefinedLookups.put("us_rank", "select rank_code ,rank_name_ar From kbrank where rank_code='PICKUPAGENT'" );
		userModifyTD.put("popup", "showLinkPopup({us_active},{us_rank},{us_id},{us_to_state})");
...
//		String html = "<td></td>";
//		html = "<td>";
//		html +="<button type=\"button\" class=\"btn btn-xs btn-warning\" onclick=\"popitup ('pickupCustomersPopUp?pickupmastercustomer="+hashy.get("us_id")+"' , '' , 800 ,700);\">العملاء</button>";
//		html +="</td>";
//		return html;
//	}
	...
	public String showLinkPopup(HashMap<String,String>hashy) {
		String html = "<td>";
		html +="<button type=\"button\" class=\"btn btn-xs btn-info\" onclick=\"popitup ('pickupAgentCustomersPopUp?customersusid="+hashy.get("us_id")+"' , '' , 1000 ,600);\">زبائن</button>";
		html +="<button type=\"button\" style='margin-right:5px;' "
				+ "class=\"btn btn-xs btn-dark\" "
				+ "onclick=\"popitup ('pickupAgentSpecialPricesPopUp?dlvagentidpopup="+hashy.get("us_id")+"' , "
						+ "'' , 1000 ,600);\">أسعار نقل خاصه للمندوب</button></br>";
	...
	public String doUpdate(HttpServletRequest rqs , boolean autoCommit){
		...
		boolean changeImage= false, changeId = false, changeResidenceId = false, changeContract=false;
		try{
			...
			for(String key:inputMap_ori.keySet()) {
        		...
        		if(!dataFromScreen.equalsIgnoreCase(dataFromDB))
        			CoreUtilities.logChanges(conn, "KBUSERS", "us_id", Integer.parseInt(keyCol), key, dataFromDB, dataFromScreen,
							"update", "إعدادات مندوبين الأستلام", usid);			
        		}
			...
			pst = conn.prepareStatement("update kbusers set us_loginid =?, us_password =? , us_active=? , us_p_b4_enc=?,"
					+ " us_name=? , us_hp=? ,  us_branchcode=?,  us_cartype=?, us_carplateno=? where us_id=?");
			...
			pst.setString(7, inputMap_ori.get("us_branchcode")[0]);
			pst.setString(8, inputMap_ori.get("us_cartype")[0]);
			pst.setString(9, inputMap_ori.get("us_carplateno")[0]);
	...
	public String doInsert(HttpServletRequest rqs , boolean autoCommit){
		...
		try{
			...
			rs.next();
			CoreUtilities.logChanges(conn, "KBUSERS", "us_id", rs.getInt(1),	 "*", 	"NONE"	, "ALL", "insert", "إعدادات مندوبين الأستلام", usid);
			conn.commit();
		}catch(Exception e){
			/*log error*/
	...
	public String doDelete(HttpServletRequest rqs) {
		...
		String msg = "";
		try {
			pst = conn.prepareStatement("select 1 from p_cases where c_pickupagent=? limit 1");
			...
			CoreUtilities.logChanges(conn, "KBUSERS", "us_id", Integer.parseUnsignedInt(rqs.getParameter(keyCol)), "*", "ALL", "NONE", "delete", "إعدادات مندوبين الأستلام", userid);
			conn.commit();
		}else {
