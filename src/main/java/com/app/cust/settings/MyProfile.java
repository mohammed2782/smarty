...
public class MyProfile extends CoreMgr{
	public MyProfile() {
		
		MainSql = "select us_hp,'' as customer, us_id, DATE_FORMAT(us_createddt + INTERVAL 8 HOUR,'%Y-%m-%d %H:%i') as us_createddt,"
				+ " us_loginid, us_password, us_active,"
				+ " us_p_b4_enc, us_rank ,"
				+ " us_name, ifnull(us_branchcode,'') as us_branchcode from kbusers where "
				+ " us_id = {userid} and us_branchcode='{userstorecode}' "
				+ " ";
	   ...
	   userDefinedNewCaption = "إضافة بيانات المستخدم";
	   userDefinedUpdateCaption = "تعديل بيانات المستخدم";
		...
		userDefinedLookups.put("us_rank", "select rank_code ,rank_name_ar From kbrank where "
				+ " rank_code not in ('LIAISONAGENT', 'DLVAGENT' , 'IT') " );
...
//		try{
//			if(ut.checkPasswordSmallOrContainSpace(inputMap.get("us_password")[0]))
//				throw new Exception ("قد يحوي الرمز على اقل من اربع احرف اويحوي مسافة فارغة");
//			if(ut.checkUserLoginIdExistOrSmall(conn, inputMap.get("us_loginid")[0],"INSERT"))
...
//							+ "  us_name 	  , us_hp	 , us_branchcode, us_createdby ) "
//					+ " values (?			 , MD5(?)	  , ?		 , ?			, ? , "
...
//			pst.setString(6, inputMap.get("us_name")[0]);
//			pst.setString(7, inputMap.get("us_hp")[0]);
//			pst.setInt(8, userstorecode);
//			pst.setInt(9, usid);
//			pst.executeUpdate();
//			rs = pst.getGeneratedKeys();
//			rs.next();
//			ut.logChanges(conn, "KBUSERS", "us_id", rs.getInt(1),	 "*", 	"NONE"	, "ALL", "insert", "مستخدمون النظام", usid);
//			conn.commit();
//		}catch(Exception e){
//			/*log error*/
...
//			pst.executeUpdate();
//			ut.logChanges(conn, "KBUSERS", "us_id", Integer.parseUnsignedInt(rqs.getParameter(keyCol)), "*", "ALL", "NONE", "delete", "مستخدمون النظام", userid);
//			conn.commit();
