...
public class ReadyToPrint extends CoreMgr{
	private int row=1;
	...
	public ReadyToPrint () {
		...
		MainSql = "select c_products, c_id, c_custreceiptnoori,c_custid, cust_name, c_productinfo, c_qty, c_rcv_name,  c_receiptamt,'' as q_action,   "
				...
				+ " where q_stage = 'NEWCUSTLOGI' and q_step = 'READYTOPRINT' and c_custid in ({shopsCommaSeperated})";
		...
		userDefinedLookups.put("q_action", "select stpd_code, stpd_desc from kbstep_decision where "
				+ " stpd_stpid in (select stp_id from kbstep where stp_code='READYTOPRINT' and stp_stgcode='NEWCUSTLOGI') and stpd_onlymbapp='N' "
	...
	public void initialize(HashMap smartyStateMap){
		...
		userDefinedCaption = "جاهز للطبع";
		String printGlobalBtn = "<button type='input' onclick='globalSellBillPrintBtn(this, event)' "
				+ " class='btn btn-sm btn-danger' >طباعة كل الفواتير المحددة <i class=\"fa fa-print fa-lg\"></i></button><span id='totalselectedcases' "
				+ " style='margin-right: 15px;'>0</span>";
		
		String assignProces = "<input type=\"checkbox\" id='selectAllGlobal' class=\"flat\" onclick='checkAllCheckBoxes(this)'>"+
				"                        <label for=\"allassign\">" + 
				"                            تحديد الكل " + 
				"                        </label>"; 
			
		userDefinedCaption ="<div class='col-md-4 col-sm-2 col-xs-2'>"+this.userDefinedCaption+"</div>"
				+"<div class='col-md-2 col-sm-4 col-xs-4'>"+assignProces+"</div>"
	...
	public String doUpdate (HttpServletRequest rqs , boolean commit) {
		...
		for (int i=1 ; i<=rowsNo ; i++){
			...
			if (action!=null && !action.trim().isEmpty() && !action.trim().equalsIgnoreCase("")&& !action.trim().equalsIgnoreCase("null")) {
				...
				if (action.equalsIgnoreCase("goreadypickup")) {
					cIdList.add(id);
		...
		try{
			if (moveCases) {
				//Connection conn,int masterCustId, int custId, int userId, int branchId
				manifestId = utn.generatePickUpManifestIdForCases(conn, mastercustidlogin, userId, currentBranch);
				utn.assignPickUpManifestIdToCases(conn, manifestId, cIdList);
			}
			pst = conn.prepareStatement("select ifnull(c_billprinted,'N') as c_billprinted from  p_cases where c_id = ?");
			...
			for (int cid :cIdList){
				...
				rs = pst.executeQuery();
				if (rs.next()) 
					if (rs.getString("c_billprinted").equalsIgnoreCase("Y"))
						printed = true;
				...
				fu.MoveDecisionStepNext(conn, cid, actionsMap.get(cid), userId, "NEWCUSTLOGI", "READYTOPRINT", "" );
			}
			conn.commit();
		}catch(Exception e){
			e.printStackTrace();
