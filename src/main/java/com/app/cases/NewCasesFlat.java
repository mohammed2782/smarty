...
public class NewCasesFlat extends CoreMgr {
	...
	public NewCasesFlat() {
		...
		userDefinedNewCaption = "انشاء شحنات فلات";
...
super.initialize(smartyStateMap);
	...
	public StringBuilder getRCVDetailsRow(int rcvSeq, int a_branchCode, String a_destState) {
		...
		String  destState = lu.getBranchStateCode();
		...
		try {
			...
			// 5-state
			String style3 = "text-align:right;  padding: 0 10px 0 10px;"
			...
			sb.append("<div class='" + divCLass + "' style='" + divStyle + "'>"
					...
					+ " name='rcv_city_" + userDefinedMultiNewRowExtension + "' style='" + style3 + "' required>");
			Map<String, String> lookupsmap2 = colMapValues.get("rcv_state_1");
			if (lookupsmap2 != null && !lookupsmap2.isEmpty()) {
				for (String code : lookupsmap2.keySet()) {
					if (destState.equalsIgnoreCase(code)) {
						sb.append("<option value='" + code + "' selected>" + lookupsmap2.get(code) + "</option> \n");
					}else {
						sb.append("<option value='" + code + "' >" + lookupsmap2.get(code) + "</option> \n");
					}
				}
			}
	...
	public String doInsert(HttpServletRequest rqs, boolean autocommit) {
		...
		try {
			...
			+ "	 c_receiptamt_usd  , c_creationstartpoint	 )"
			+ " values ("+CoreUtilities.getQuestionMarks(17)+")", Statement.RETURN_GENERATED_KEYS);
			CaseInformation ci = new CaseInformation();
			int i = 0;
