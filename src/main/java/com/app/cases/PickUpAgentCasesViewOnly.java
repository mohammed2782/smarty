...
public class PickUpAgentCasesViewOnly extends CoreMgr{
	 ...
	 * 
	 */
	public PickUpAgentCasesViewOnly (){
	MainSql = "select c_paid_delivery_cost_in_advance, c_rtnreason, q_postponedoption, '' as del_edit, c_receiptfromsystem, q_previous_action_taken_by , c_createdby, q_stage, q_step, stp_name, "
			+ " date(c_createddt) as c_createddt , c_id , c_custid     , c_custhp		 , "
			  ...
			  + " where 1=0 and c_pickupagent={usid} ";
	...
	public void initialize(HashMap smartyStateMap){
		...
		if (search_paramval!=null && !search_paramval.isEmpty()) {
			MainSql = "select distinct '' as del_edit, c_paid_delivery_cost_in_advance, c_receiptfromsystem, q_previous_action_taken_by , c_createdby, q_stage, q_step, stp_name, "
					+ " date(c_createddt) as c_createddt , c_id , c_custid     , c_custhp		 , "
					  + " c_rcv_name 	 , c_rcv_hp1      , c_rcv_state		 , "
					  + " c_rural        , concat (st_name_ar, '-' ,ifnull(cdi_name,''),' ' ,c_rcv_addr_rmk) c_rcv_addr_rmk, c_rmk     		 , "
					  ...
					  + " ifnull(chainfor_pmt.cc_branchpmtid,0) as cc_branchpmtid, "
					  + " (case "
					  ...
					  + " left join p_caseschain chainfor_pmt on (chainfor_pmt.cc_caseid = c_id and (chainfor_pmt.cc_frombranch=c_branchcode or chainfor_pmt.cc_tobranch=c_branchcode )  )"
					  + " left join kbbranches on q_branch = branch_id "
					  + " left join kbrtn_reasons on (rtn_code = c_rtnreason)"
					  + " left join kbstate on st_code = c_rcv_state and st_branch = c_branchcode "
					  + " left join kbcity_district on (cdi_id =c_rcv_district and cdi_stcode=c_rcv_state) "
					  + " where c_pickupagent="+lu.getUsid()+" ";
		}
	...
	public String modifyStepName(HashMap<String,String>hashy) {
		...
		if(hashy.get("q_stage").equalsIgnoreCase("DLV")) {
			...
			if (lu.getRank_code().equalsIgnoreCase("ITBOSS")) {
				forceDlvBtn ="<button type=\"button\" onclick='forceDlv("+hashy.get("c_id")+")' "
		...
		String actionMenu ="<div class='dropdown'>"
				+ "<button type='button' style='background-color:"+hashy.get("stp_color")+";' "
						+ " class='btn btn-light dropdown-toggle btn-sm' type=\"button\" "
						+ " data-bs-toggle=\"dropdown\" aria-expanded=\"false\">"+buttonText+"</button>"
				+ " <ul class='dropdown-menu'>";
		
		
		actionMenu +="<li>"+audit+"</li></ul></div>";
		String html = "<td><div class='btn-group mr-1 mb-1'>"+actionMenu;
		
		html+= "</div>"+forceDlvBtn+"</td>";
		*/
		//desc +="&nbsp;"+actionMenu;
		...
		html+= "</div>";
		if (lu.getRank_code().equalsIgnoreCase("ITBOSS")) {
			html +="<button type=\"button\" onclick='forceDlv("+hashy.get("c_id")+")' "
					+ " class=\"btn btn-warning radius-30 btn-sm\" style='margin-top:5px;'>احتسابه واصل</button>";
		}
		html+= "</td>";
