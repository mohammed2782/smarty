...
public class NewSpecialCasesBarcode extends CoreMgr {
	...
	public NewSpecialCasesBarcode() {
		...
		userDefinedNewCaption = "إنشاء شحنة خاصة";
	...
	public StringBuilder getRCVDetailsRow(int rcvSeq, String a_receiptNo, int a_branchCode) {
		StringBuilder sb = new StringBuilder();
		...
		try {
			conn1 = mysql.getConn();
			try {
				receiptInSystem = ut.checkIfReceiptGeneratedFromSystem(conn1, a_receiptNo, a_branchCode);
				if (!receiptInSystem)
					throw new Exception ("الوصل رقم "+a_receiptNo+" غير متولد من النظام");
				
				receiptUsedBefore = ut.checkIfReceiptUsedBefore(conn1, a_receiptNo);
				if (receiptUsedBefore)
					throw new Exception ("الوصل رقم "+a_receiptNo+" تم أستعماله سابقا");
				
				custId = ut.getCustomerOwnerOfReceipt(conn1, a_receiptNo);
			} catch (Exception e) {
				throw e;
			}
...
//			// 7-agent share cost	
//			sb.append("<div class='" + divCLass + "' style='" + divStyle + "'>" // style='margin-left:1%;margin-right:1%;'
//					+ "<label class='" + labelClass + "'>أجور المندوب / الفرع</label>" +"<i class=\"ft-package\"></i>"
...
//					+ "</div>");
			...
			// 8-shipment cost	
			sb.append("<div class='" + divCLass + "' style='" + divStyle + "'>"
					+ "<label class='" + labelClass + "'>مبلغ الشحن الكلي</label>"
	...
	public String doInsert(HttpServletRequest rqs, boolean autocommit) {
		...
		try {
			...
			try {
				...
				+ "	 c_receiptamt_usd, c_custid 	 , c_custhp		  , c_agentshare , c_specialcase,"
				+ "  c_receiptfromsystem)"
				+ " values ("+CoreUtilities.getQuestionMarks(19)+"	  , 'Y' , 'Y')", Statement.RETURN_GENERATED_KEYS);
		
				pstUpdateReciept 	= 
						conn.prepareStatement("update p_receipts "
								+ " set rec_used='Y', rec_caseid=?, rec_assigned_master_cust=?, rec_assigned_customer=? "
								+ " where rec_full_receipt_id=?  ");
				pstUpdateBook = 
						conn.prepareStatement("update "
								+ " p_receipts_books join p_receipts on rbook_setprefix =  rec_set_prefix and  rbook_no=rec_receipt_book_no   "
								+ " set rbook_used='Y' where rec_full_receipt_id=? " );
				...
				for (int i = 0; i < cases.size(); i++) {
					...
					pst.setDouble(19, 0); // changed from  ci.getAgentShare() to 0
					pst.executeUpdate();
					...
					pstUpdateReciept.setInt(2, ci.getMasterSenderId());
					pstUpdateReciept.setInt(3, ci.getSenderId());
					pstUpdateReciept.setString(4, ci.getCustReceiptNoOri());
