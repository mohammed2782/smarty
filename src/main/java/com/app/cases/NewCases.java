...
public class NewCases extends CoreMgr {
	...
	public StringBuilder getRCVDetailsRow(int rcvSeq, String a_receiptNo, int a_branchCode) throws Exception{
		...
		try {
			...
			if (custId>0) {
				String custName = Utilities.getCustomerName(conn1, custId);
	...
	public String doInsert(HttpServletRequest rqs, boolean autocommit) {
		...
		try {
			...
			+ "	 c_receiptamt_usd, c_custid 	 , c_custhp		  , c_paid_delivery_cost_in_advance, c_rcv_name,"
			+ "  c_receiptfromsystem)"
			+ " values ("+CoreUtilities.getQuestionMarks(20)+", 'Y')", Statement.RETURN_GENERATED_KEYS);
				...
				for (int i = 0; i < cases.size(); i++) {
					...
					pst.setString(20, ci.getReceiverName());
					pst.executeUpdate();
